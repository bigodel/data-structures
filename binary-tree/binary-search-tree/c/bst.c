...
bool insertNodeRec(Node *newNode, Node **root)
{
    ...
    if (newNode->key < (*root)->key) {
        bool inserted = insertNodeRec(newNode, &(*root)->left);
        ...
        bool inserted = insertNodeRec(newNode, &(*root)->right);
...
{
    Node *res = searchRec(key, tree->root);

    if (res == NULL) {
        fprintf(stderr, "Did not find key %i in this tree.\n", key);
    }
...
bool removeNodeRec(int key, Node **root)
{
    ...
    if (key < (*root)->key) {
        bool removed = removeNodeRec(key, &(*root)->left);
        ...
        bool removed = removeNodeRec(key, &(*root)->right);
